...
func main() {
	startTime := time.Now()
	//list := []int{3, 4, 7, 2, 5, 7, 8, 4, 6, 8, 6, 3, 
		//66, 432, 63, 6, 7, 8, 4, 65, 34, 4, 36}
	...
	err = chic.QueueBind(queueForConfirm.Name,queueForConfirm.Name,
	...
	queueForResult, err := chir.QueueDeclare("",
		false,true,true,false,nil)
	failOnError(err, "Failed to declare a queue")
	err = chir.QueueBind(queueForResult.Name,queueForResult.Name,
		exchangeName,false,nil)
	failOnError(err, "Failed to bind a queue")
	...
	err = cho.Publish(exchangeName,"generator",false,false,
		amqp.Publishing{
	...
	failOnError(err, "Failed to encode")
	msg := listB
	err = cho.Publish(exchangeName,targetKey,false,false,
		amqp.Publishing{
			ContentType: "text/plain",
			Body:        msg,
		})
