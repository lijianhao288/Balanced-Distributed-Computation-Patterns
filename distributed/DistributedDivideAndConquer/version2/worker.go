...
func main() {
	...
	err = chi.QueueBind(queueIn.Name,
		queueIn.Name,exchangeName,false,nil)
	...
	if length := len(list); length <= 4 {
		...
		failOnError(err, "Failed to bind a queue")
		leftConfirmMsgs, err := chilc.Consume(
			queueLeftConfirm.Name,"",false,true,false,false,nil)
		failOnError(err, "Failed to register a consumer")
		...
		failOnError(err, "Failed to bind a queue")
		rightConfirmMsgs, err := chirc.Consume(queueRightConfirm.Name,
			"",false,true,false,false,nil)
		failOnError(err, "Failed to register a consumer")
		...
		failOnError(err, "Failed to bind a queue")
		rightResultMsgs, err := chirr.Consume(queueRightResult.Name,
			"",false,false,false,false,nil)
		failOnError(err, "Failed to register a consumer")
		//send args
		argsLeftP := &workerArg{
		...
		//final result
		finalResult := Merge(listLeftResult, listRightResult)
