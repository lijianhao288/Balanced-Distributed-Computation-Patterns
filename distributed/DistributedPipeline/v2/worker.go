...
func main() {
	...
	if workerType == "startworker" {
		w := startWorker{Worker{conn1, conn2, conn3, conn4, nil, nil, nil,
			exchangeName, inputConfirmQueue, function}}
		w.WaitNext()
		w.Work()
	} else if workerType == "midworker" {
		w := midWorker{Worker{conn1, conn2, conn3, conn4, nil, nil, nil,
			exchangeName, inputConfirmQueue, function}}
		w.ConnectPrevious()
		w.WaitNext()
		w.Work()
	} else {
		w := endWorker{Worker{conn1, conn2, conn3, conn4, nil, nil, nil,
			exchangeName, inputConfirmQueue, function}}
		w.ConnectPrevious()
		w.Work()
	}
...
func (w *Worker) ConnectPrevious() {
	...
	err = chis.Publish(w.exchangeName, 
	    w.inputConfirmQueue,false, false,
		amqp.Publishing{
...
func (w *Worker) WaitNext() {
	...
	failOnError(err, "Failed to register a consumer")
	//print out the confirm queue for next worker
	fmt.Println(
		"The input confirm queue (2nd cmd line arg for the next worker):", 
